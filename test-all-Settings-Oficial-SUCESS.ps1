...
$settingsUrl = "$baseUrl/me/settings"
$adminSettingsUrl = "$baseUrl/settings"
$publicSettingsUrl = "$baseUrl/public/settings"
...
# ============================================
#  STEP 1: REGISTRO DE NOVO USUARIO
# ============================================

Write-Host "`n[SETUP] Registrando novo usuário..." -ForegroundColor Cyan

$uniqueId = Get-Date -Format "yyyyMMddHHmmssfff"
$testEmail = "test-$uniqueId@leadflow.dev"
...
try {
    $registerResp = Invoke-WebRequest -Uri $registerUrl `
        -Method Post `
        -ContentType "application/json" `
        -Body $registerData `
        -UseBasicParsing
    
    $registerBody = $registerResp.Content | ConvertFrom-Json
    $tenantId = $registerBody.tenantId
    Write-Host "✅ Usuário registrado: $testEmail" -ForegroundColor Green
...
Write-Host "`n[SETUP] Fazendo login..." -ForegroundColor Cyan
...
Write-Host "Debug: URL = $settingsUrl" -ForegroundColor DarkGray
Write-Host "Debug: Headers = $($headers | ConvertTo-Json)" -ForegroundColor DarkGray
...
try {
    ...
    Write-Host "ERRO - Status: $status" -ForegroundColor Red
    try {
...
# ============================================
# TESTE 5: PUT /api/settings/{id} (ADMIN)
# ============================================

Write-Host "`n[5/9] PUT /api/settings/{id} (Update by ID - Admin)" -ForegroundColor Cyan
...
try {
    $resp = Invoke-WebRequest -Uri "$adminSettingsUrl/$settingId" `
        -Method Put `
        -ContentType "application/json" `
        -Headers $headers `
        -Body $updateData `
...
# ============================================
# TESTE 6: GET /public/settings/{id}
# ============================================

Write-Host "`n[6/9] GET /public/settings/{id} (Public access)" -ForegroundColor Cyan

try {
    $resp = Invoke-WebRequest -Uri "$publicSettingsUrl/$settingId" `
        -Method Get `
        -UseBasicParsing
...
# ============================================
# TESTE 8: DELETE /api/settings/{id} (ADMIN)
# ============================================

Write-Host "`n[8/9] DELETE /api/settings/{id} (Delete by ID - Admin)" -ForegroundColor Cyan
