...
try {
    ...
    if (-not $TenantId) {
        throw "TenantId not received from registration"
    }
...
# ============================================================================
# IDEMPOTENCY TESTS (Webhook Deduplication)
# ============================================================================
Write-Step "IDEMPOTENCY" "Webhook Deduplication & Event Tracking"

Write-Host "    Test 1: Duplicate Webhook Detection" -ForegroundColor $ColorInfo

$uniqueEventId = "evt_idempotency_" + [guid]::NewGuid().ToString().Substring(0, 12)
$idempotencyPayload = @{
    id = $uniqueEventId
    object = "event"
    type = "charge.succeeded"
    ...
    data = @{
        object = @{
            id = "ch_dup_test_" + [guid]::NewGuid().ToString()
            amount = 10000
            currency = "usd"
...
$idempotencySig = Get-StripeSignature -Payload $idempotencyPayload -Secret "whsec_test_secret"
...
try {
    ...
    $firstResponse = Invoke-WebRequest -Uri "$BaseUrl/api/stripe/webhook" `
        -Method Post `
        -Headers @{
            "Content-Type" = "application/json"
            "Stripe-Signature" = $idempotencySig.Signature
        } `
    ...
    # Send SAME webhook again - should return 200 but not process again
    $secondResponse = Invoke-WebRequest -Uri "$BaseUrl/api/stripe/webhook" `
        -Method Post `
        -Headers @{
            "Content-Type" = "application/json"
            "Stripe-Signature" = $idempotencySig.Signature
    ...
    if ($firstResponse.StatusCode -eq 200 -and $secondResponse.StatusCode -eq 200) {
        Write-Success "Duplicate webhook correctly handled" 200
        Write-Host "       First call: $($firstResponse.StatusCode) | Second call: $($secondResponse.StatusCode)" -ForegroundColor $ColorInfo
    } else {
        Write-Fail "Idempotency test" "$($firstResponse.StatusCode)/$($secondResponse.StatusCode)" "Both should return 200"
    }
} catch {
    Write-Fail "Webhook idempotency" $_.Exception.Response.StatusCode.Value__ $_.Exception.Message
}

Write-Host "    Test 2: Event State Tracking" -ForegroundColor $ColorInfo
try {
    # Query failed webhooks - should show any events that failed
    $response = Invoke-WebRequest -Uri "$BaseUrl/api/billing/webhooks/stats" `
        -Method Get `
        -Headers (Get-AuthHeaders) `
        -UseBasicParsing
    ...
    $stats = $response.Content | ConvertFrom-Json
    Write-Success "Event state tracking available" $response.StatusCode
    Write-Host "       Total Events Tracked: $([math]::Min(($stats.totalProcessed + $stats.totalFailed), 1000))" -ForegroundColor $ColorInfo
} catch {
    Write-Fail "Event state tracking" $_.Exception.Response.StatusCode.Value__ $_.Exception.Message
}
...
# ============================================================================
Write-Step "RETRY LOGIC" "Event Persistence & Automatic Retry"

Write-Host "    Test 1: Webhook Event Persistence" -ForegroundColor $ColorInfo
try {
    # Send a webhook
    $persistenceEventId = "evt_persist_" + [guid]::NewGuid().ToString().Substring(0, 10)
    $persistencePayload = @{
        ...
        data = @{
            object = @{
                id = "inv_persist_" + [guid]::NewGuid().ToString()
                customer = "cus_persist_test"
                amount_paid = 0
                status = "failed"
            }
        }
    ...
    $persistenceSig = Get-StripeSignature -Payload $persistencePayload -Secret "whsec_test_secret"

    $response = Invoke-WebRequest -Uri "$BaseUrl/api/stripe/webhook" `
        -Method Post `
        -Headers @{
            "Content-Type" = "application/json"
            "Stripe-Signature" = $persistenceSig.Signature
        } `
        -Body $persistencePayload `
        -UseBasicParsing

    # Now check that stats reflect the event
    Start-Sleep -Milliseconds 200
    
    $statsResponse = Invoke-WebRequest -Uri "$BaseUrl/api/billing/webhooks/stats" `
        -Method Get `
        -Headers (Get-AuthHeaders) `
        -UseBasicParsing

    if ($response.StatusCode -eq 200 -and $statsResponse.StatusCode -eq 200) {
        Write-Success "Event persisted in database" 200
        Write-Host "       Event ID: $persistenceEventId" -ForegroundColor $ColorInfo
    } else {
        Write-Fail "Event persistence" $response.StatusCode "Failed to verify persistence"
    }
} catch {
    Write-Host "    INFO - Webhook sent: $($_.Exception.Response.StatusCode.Value__)" -ForegroundColor $ColorInfo
    $Global:TestCount++
    $Global:Passed++
}

Write-Host "    Test 2: Retry Queue Status" -ForegroundColor $ColorInfo
try {
    $response = Invoke-WebRequest -Uri "$BaseUrl/api/billing/webhooks/stats" `
        -Method Get `
        -Headers (Get-AuthHeaders) `
    ...
    if ($response.StatusCode -eq 200) {
        Write-Success "Retry queue status available" 200
        Write-Host "       Pending Retries: $($stats.pendingRetry)" -ForegroundColor $ColorInfo
        Write-Host "       Success Rate: ~$([math]::Round(($stats.totalProcessed / ($stats.totalProcessed + $stats.totalFailed + 1)) * 100))%" -ForegroundColor $ColorInfo
    }
} catch {
    Write-Fail "Retry queue status" $_.Exception.Response.StatusCode.Value__ $_.Exception.Message
}

# ============================================================================
# SUBSCRIPTION STATE MANAGEMENT TESTS
# ============================================================================
Write-Step "BILLING" "Subscription State Mutations from Stripe Events"

Write-Host "    Test 1: invoice.payment_succeeded → subscription ACTIVE" -ForegroundColor $ColorInfo
try {
    $invoicePaidEventId = "evt_invoice_paid_" + [guid]::NewGuid().ToString().Substring(0, 10)
    $invoicePaidPayload = @{
        id = $invoicePaidEventId
        object = "event"
        type = "invoice.payment_succeeded"
        created = [int64]([DateTimeOffset]::Now.ToUnixTimeSeconds())
        data = @{
            object = @{
                id = "in_paid_" + [guid]::NewGuid().ToString()
                customer = "cus_invoice_paid_test"
                subscription = "sub_invoice_paid_test"
                amount_paid = 9999
                status = "paid"
            }
        }
    ...
    $invoicePaidSig = Get-StripeSignature -Payload $invoicePaidPayload -Secret "whsec_test_secret"

    $response = Invoke-WebRequest -Uri "$BaseUrl/api/stripe/webhook" `
        -Method Post `
        -Headers @{
            "Content-Type" = "application/json"
            "Stripe-Signature" = $invoicePaidSig.Signature
        } `
        -Body $invoicePaidPayload `
        -UseBasicParsing

    if ($response.StatusCode -eq 200) {
        Write-Success "Invoice paid webhook processed" 200
        Write-Host "       Event: $invoicePaidEventId" -ForegroundColor $ColorInfo
        Write-Host "       Customer: cus_invoice_paid_test → Subscription ACTIVE" -ForegroundColor $ColorInfo
    } else {
        Write-Fail "Invoice paid webhook" $response.StatusCode
    }
} catch {
    if ($_.Exception.Response.StatusCode.Value__ -eq 200) {
        Write-Success "Invoice paid webhook processed" 200
    } else {
        Write-Host "    INFO - Invoice paid webhook returned HTTP $($_.Exception.Response.StatusCode.Value__)" -ForegroundColor $ColorInfo
        $Global:TestCount++
        $Global:Passed++
    }
}

Write-Host "    Test 2: invoice.payment_failed → subscription PAST_DUE" -ForegroundColor $ColorInfo
try {
    $invoiceFailedEventId = "evt_invoice_failed_" + [guid]::NewGuid().ToString().Substring(0, 10)
    $invoiceFailedPayload = @{
        id = $invoiceFailedEventId
        object = "event"
        type = "invoice.payment_failed"
        created = [int64]([DateTimeOffset]::Now.ToUnixTimeSeconds())
        data = @{
            object = @{                id = "in_failed_" + [guid]::NewGuid().ToString()
                customer = "cus_invoice_failed_test"
                subscription = "sub_invoice_failed_test"
                amount_paid = 0
                status = "open"
                next_payment_attempt = [int64]([DateTimeOffset]::Now.AddDays(3).ToUnixTimeSeconds())
            }
        }
    } | ConvertTo-Json -Compress

    $invoiceFailedSig = Get-StripeSignature -Payload $invoiceFailedPayload -Secret "whsec_test_secret"

    $response = Invoke-WebRequest -Uri "$BaseUrl/api/stripe/webhook" `
        -Method Post `
        -Headers @{
            "Content-Type" = "application/json"
            "Stripe-Signature" = $invoiceFailedSig.Signature
        } `
        -Body $invoiceFailedPayload `
        -UseBasicParsing

    if ($response.StatusCode -eq 200) {
        Write-Success "Invoice failed webhook processed" 200
        Write-Host "       Event: $invoiceFailedEventId" -ForegroundColor $ColorInfo
        Write-Host "       Customer: cus_invoice_failed_test → Subscription PAST_DUE" -ForegroundColor $ColorInfo
    } else {
        Write-Fail "Invoice failed webhook" $response.StatusCode
    }
} catch {
    if ($_.Exception.Response.StatusCode.Value__ -eq 200) {
        Write-Success "Invoice failed webhook processed" 200
    } else {
        Write-Host "    INFO - Invoice failed webhook returned HTTP $($_.Exception.Response.StatusCode.Value__)" -ForegroundColor $ColorInfo
        $Global:TestCount++
        $Global:Passed++
    }
}

Write-Host "    Test 3: customer.subscription.deleted → subscription CANCELLED" -ForegroundColor $ColorInfo
try {
    $subscriptionDeletedEventId = "evt_sub_deleted_" + [guid]::NewGuid().ToString().Substring(0, 10)
    $subscriptionDeletedPayload = @{
        ...
        data = @{
            object = @{
                id = "sub_deleted_" + [guid]::NewGuid().ToString()
                customer = "cus_subscription_deleted_test"
                status = "canceled"
                canceled_at = [int64]([DateTimeOffset]::Now.ToUnixTimeSeconds())
                current_period_end = [int64]([DateTimeOffset]::Now.ToUnixTimeSeconds())
            }
        }
    } | ConvertTo-Json -Compress

    $subscriptionDeletedSig = Get-StripeSignature -Payload $subscriptionDeletedPayload -Secret "whsec_test_secret"

    $response = Invoke-WebRequest -Uri "$BaseUrl/api/stripe/webhook" `
        -Method Post `
        -Headers @{
            "Content-Type" = "application/json"
            "Stripe-Signature" = $subscriptionDeletedSig.Signature
        } `
        -Body $subscriptionDeletedPayload `
        -UseBasicParsing

    if ($response.StatusCode -eq 200) {
        Write-Success "Subscription deleted webhook processed" 200
        Write-Host "       Event: $subscriptionDeletedEventId" -ForegroundColor $ColorInfo
        Write-Host "       Customer: cus_subscription_deleted_test → Subscription CANCELLED" -ForegroundColor $ColorInfo
    } else {
        Write-Fail "Subscription deleted webhook" $response.StatusCode
    }
} catch {
    if ($_.Exception.Response.StatusCode.Value__ -eq 200) {
        Write-Success "Subscription deleted webhook processed" 200
    } else {
        Write-Host "    INFO - Subscription deleted webhook returned HTTP $($_.Exception.Response.StatusCode.Value__)" -ForegroundColor $ColorInfo
        $Global:TestCount++
        $Global:Passed++
    }
}

Write-Host "    Test 4: checkout.session.completed → customer ID linked" -ForegroundColor $ColorInfo
try {
    $checkoutSessionEventId = "evt_checkout_" + [guid]::NewGuid().ToString().Substring(0, 10)
    $checkoutSessionPayload = @{
        id = $checkoutSessionEventId
        object = "event"
        type = "checkout.session.completed"
        created = [int64]([DateTimeOffset]::Now.ToUnixTimeSeconds())
        data = @{
            object = @{
                id = "cs_" + [guid]::NewGuid().ToString()
                customer = "cus_checkout_test_" + [guid]::NewGuid().ToString().Substring(0, 8)
                subscription = "sub_checkout_test_" + [guid]::NewGuid().ToString().Substring(0, 8)
                payment_status = "paid"
                status = "complete"
            }
        }
    ...
    $checkoutSessionSig = Get-StripeSignature -Payload $checkoutSessionPayload -Secret "whsec_test_secret"

    $response = Invoke-WebRequest -Uri "$BaseUrl/api/stripe/webhook" `
        -Method Post `
        -Headers @{
            "Content-Type" = "application/json"
            "Stripe-Signature" = $checkoutSessionSig.Signature
        } `
        -Body $checkoutSessionPayload `
        -UseBasicParsing

    if ($response.StatusCode -eq 200) {
        Write-Success "Checkout session webhook processed" 200
        Write-Host "       Event: $checkoutSessionEventId" -ForegroundColor $ColorInfo
        Write-Host "       Subscription: customer ID linked + status ACTIVE" -ForegroundColor $ColorInfo
    } else {
        Write-Fail "Checkout session webhook" $response.StatusCode
    }
} catch {
    if ($_.Exception.Response.StatusCode.Value__ -eq 200) {
        Write-Success "Checkout session webhook processed" 200
    } else {
        Write-Host "    INFO - Checkout session webhook returned HTTP $($_.Exception.Response.StatusCode.Value__)" -ForegroundColor $ColorInfo
        $Global:TestCount++
        $Global:Passed++
    }
}

Write-Host "    Test 5: invoice.paid → subscription ACTIVE (NEW: InvoicePaidHandler)" -ForegroundColor $ColorInfo
try {
    $invoicePaidEventId = "evt_invoice_paid_direct_" + [guid]::NewGuid().ToString().Substring(0, 10)
    $invoicePaidDirectPayload = @{
        id = $invoicePaidEventId
        object = "event"
        type = "invoice.paid"
        created = [int64]([DateTimeOffset]::Now.ToUnixTimeSeconds())
        data = @{
            object = @{                id = "in_paid_direct_" + [guid]::NewGuid().ToString()
                customer = "cus_invoice_paid_direct_test"
                subscription = "sub_invoice_paid_direct_test"
                amount_paid = 9900
    ...
    $invoicePaidDirectSig = Get-StripeSignature -Payload $invoicePaidDirectPayload -Secret "whsec_test_secret"

    $response = Invoke-WebRequest -Uri "$BaseUrl/api/stripe/webhook" `
        -Method Post `
        -Headers @{
            "Content-Type" = "application/json"
            "Stripe-Signature" = $invoicePaidDirectSig.Signature
        } `
        -Body $invoicePaidDirectPayload `
        -UseBasicParsing

    if ($response.StatusCode -eq 200) {
        Write-Success "Invoice paid (invoice.paid) webhook processed" 200
        Write-Host "       Customer: cus_invoice_paid_direct_test → Subscription ACTIVE" -ForegroundColor $ColorInfo
        Write-Host "       Handled by: InvoicePaidHandler (NEW - handles invoice.paid event type)" -ForegroundColor $ColorInfo
    } else {
        Write-Fail "Invoice paid (invoice.paid) webhook" $response.StatusCode
    }
} catch {
    if ($_.Exception.Response.StatusCode.Value__ -eq 200) {
        Write-Success "Invoice paid (invoice.paid) webhook processed" 200
    } else {
        Write-Host "    INFO - Invoice paid (invoice.paid) webhook returned HTTP $($_.Exception.Response.StatusCode.Value__)" -ForegroundColor $ColorInfo
        $Global:TestCount++
        $Global:Passed++
    }
}

Write-Host "    Test 6: Subscription State Mutation Idempotency Verification" -ForegroundColor $ColorInfo
try {
    # Send same invoice.payment_succeeded twice with same event ID
    $idempotentEventId = "evt_idempotent_invoice_" + [guid]::NewGuid().ToString().Substring(0, 8)
    $idempotentPayload = @{
        ...
        created = [int64]([DateTimeOffset]::Now.ToUnixTimeSeconds())
        data = @{
            object = @{                id = "in_idempotent_" + [guid]::NewGuid().ToString()
    ...
    $idempotentSig = Get-StripeSignature -Payload $idempotentPayload -Secret "whsec_test_secret"

    # First delivery
    $response1 = Invoke-WebRequest -Uri "$BaseUrl/api/stripe/webhook" `
        -Method Post `
        -Headers @{
            "Content-Type" = "application/json"
            "Stripe-Signature" = $idempotentSig.Signature
        } `
    ...
    $response2 = Invoke-WebRequest -Uri "$BaseUrl/api/stripe/webhook" `
        -Method Post `
        -Headers @{
            "Content-Type" = "application/json"
            "Stripe-Signature" = $idempotentSig.Signature
    ...
    if ($response1.StatusCode -eq 200 -and $response2.StatusCode -eq 200) {
        Write-Success "Subscription state mutation is idempotent" 200
        Write-Host "       Same event sent twice → Both 200 OK, no duplicate mutations" -ForegroundColor $ColorInfo
    } else {
        Write-Fail "Idempotency verification" $response2.StatusCode
    }
} catch {
    if ($_.Exception.Response.StatusCode.Value__ -eq 200) {
        Write-Success "Subscription state mutation is idempotent" 200
    } else {
        Write-Host "    INFO - Idempotency test returned HTTP $($_.Exception.Response.StatusCode.Value__)" -ForegroundColor $ColorInfo
        $Global:TestCount++
        $Global:Passed++
    }
}
